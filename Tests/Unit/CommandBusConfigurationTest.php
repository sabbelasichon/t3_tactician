...
class CommandBusConfigurationTest extends UnitTestCase
{
    ...
    public function testAddCommandBusConfigurationSuccessfully()
    {
        $activePackage = $this->prophesize(PackageInterface::class);
        $activePackage->getPackagePath()
            ->willReturn('foo/');

        $activePackages = [$activePackage->reveal()];
        ...
        $this->packageManager->getActivePackages()
            ->willReturn($activePackages);
        $this->filesystem->isFile('foo/Configuration/CommandBus.php')
            ->willReturn(true);
        $this->filesystem->require('foo/Configuration/CommandBus.php')
            ->willReturn($commandBusConfiguration);

        $subject = new CommandBusConfiguration(
            '_default',
            $this->packageManager->reveal(),
    ...
    public function testAddMultipleCommandBusConfigurationWithSameCommandHandlerThrowsException()
    {
        $activePackage1 = $this->prophesize(PackageInterface::class);
        $activePackage1->getPackagePath()
            ->willReturn('foo/');

        $activePackage2 = $this->prophesize(PackageInterface::class);
        $activePackage2->getPackagePath()
            ->willReturn('bar/');

        $activePackages = [$activePackage1->reveal(), $activePackage2->reveal()];
        ...
        $this->filesystem->isFile('bar/Configuration/CommandBus.php')
            ->willReturn(true);
        $this->filesystem->require('bar/Configuration/CommandBus.php')
            ->willReturn($commandBusConfiguration);
